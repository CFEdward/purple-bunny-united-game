function reset_variables() {
	keyLeft = 0;
	keyRight = 0;
	keyUp = 0;
	keyDown = 0;
	vSpeed = 0
	hsp = 0
}

function get_input(){
	if keyboard_check (ord("A")) || keyboard_check(vk_left)	 keyLeft	= 1;
	if keyboard_check (ord("D")) || keyboard_check(vk_right) keyRight	= 1;
	if keyboard_check (ord("W")) || keyboard_check(vk_up)	 keyUp		= 1;
	if keyboard_check (ord("S")) || keyboard_check(vk_down)	 keyDown	= 1;

	keyActivate =	 keyboard_check_pressed(ord("E"));
	keyAttack =		 mouse_check_button_pressed(mb_left);
	keyRoll =		 keyboard_check_pressed(vk_space);
	
}

function calc_movement() {
	 hsp = keyRight - keyLeft;
	 vSpeed = keyDown - keyUp;
	 
	 var _facing = (aim_dir < 90 or aim_dir > 270);
	 if _facing == 0 _facing = -1
	 facing = _facing;
	
	if hsp != 0 or vSpeed != 0 {
		//get direction we are moving
		var _dir = point_direction(0, 0, hsp, vSpeed);
		
		//get distance we are moving
		hsp = lengthdir_x(speedWalk, _dir);
		vSpeed = lengthdir_y(speedWalk, _dir);
		
		//add movement to players position 
		x += hsp;
		y += vSpeed;
		
	}
	
	x += hsp;
	y += vsp;
	
	//apply knockback drag
	switch(state) {
		default: var _drag = 0.15; break;
		case states.DEAD: var _drag = 0.08; break;
	}
	hsp = lerp(hsp, 0, _drag);
	vsp = lerp(vsp, 0, _drag);
	
}

function aim_bow() {
	aim_dir = point_direction(x, y, mouse_x, mouse_y);
	my_bow.image_angle = aim_dir;	
} 

function collision() {
	//set values
	var _tx = x;
	var _ty = y;
	
	//move back last step position, out of the collision
	x = xprevious;
	y = yprevious;
	
	//get distance we want to move
	var _disx = ceil(abs(_tx - x));
	var _disy = ceil(abs(_ty - y));
	
	if place_meeting(x + _disx * sign(_tx - x), y, o_wall) x = round(x);
	if place_meeting(x, y + _disy * sign(_ty - y), o_wall) y = round(y);
	
	//move as far in x and  y before hitting the wall
	repeat(_disx) {
		if !place_meeting(x + sign(_tx - x), y, o_wall) x += sign(_tx - x);	
	}
	repeat(_disy) {
		if !place_meeting(x, y + sign(_ty - y), o_wall) y += sign(_ty - y);	
	}
}








function anim() {
	switch(state) {
		default:
			if hsp != 0 or vSpeed != 0 {
				sprite_index = sMainCharacter   //s_player_walk; 
			} else {
				sprite_index = sMainCharacter   //s_player_idle;	
			}
		break;
		case states.DEAD:
			sprite_index = sMainCharacter        //s_player_dead;
		break;
	}
}

function check_fire() {

	if mouse_check_button(mb_left){
	
		if can_attack {
			can_attack = false;
			alarm[CAN_ATTACK] = fire_rate;
			
			var _dir = point_direction(x, y, mouse_x, mouse_y);
			bow_dis = 5;
			
			var _inst = instance_create_layer(x, y, "Arrow", o_arrow);
			with(_inst) {
				speed = other.arrow_speed;	
				direction = _dir;
				image_angle = _dir;
				owner_id = other;
			}
		}
	
	}

}




















